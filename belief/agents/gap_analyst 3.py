...
class GapAnalystAgent(BaseAgent):
    ...
    async def run(self, state: UnifiedState) -> UnifiedState:
        ...
        try:
            ...
            prev_fmt = "\n".join(f"  [{i+1}] {s}" for i, s in enumerate(reversed(prev))) if prev else "  (first iteration)"
            ...
            prompt = GAP_ANALYST_PROMPT.format(
                goal=spec.goal, goal_refined=spec.goal_refined,
                ...
                exec_stdout=(exec_result.stdout[-1000:] if exec_result and exec_result.stdout else "(empty)"),
                exec_stderr=(exec_result.stderr[-1000:] if exec_result and exec_result.stderr else "(empty)"),
                exec_error_summary=(exec_result.error_summary if exec_result else "no execution data"),
            ...
            llm_report = await llm.generate_structured(
                role=self.role, system=GAP_ANALYST_SYSTEM, prompt=prompt,
                response_schema=GapReport, temperature=0.2,
            ...
            llm_report.total_blockers = sum(1 for g in llm_report.gaps if g.severity == GapSeverity.BLOCKER)
            llm_report.total_major = sum(1 for g in llm_report.gaps if g.severity == GapSeverity.MAJOR)
            llm_report.requires_research = any(g.requires_new_research for g in llm_report.gaps)

            state.gap_report = llm_report
            logger.info(f"Gap analysis: {llm_report.total_blockers} blockers, {llm_report.total_major} major")
...
def _check_basic_gaps(state: UnifiedState) -> list[Gap]:
    ...
    if not state.code_files:
        gaps.append(Gap(description="No code files produced", severity=GapSeverity.BLOCKER, category="functionality"))
        return gaps

    for fname, content in state.code_files.items():
        if "# TODO" in content or "pass  # placeholder" in content:
            gaps.append(Gap(description=f"'{fname}' contains TODO/placeholder", severity=GapSeverity.MAJOR, category="functionality"))

    exec_result = state.execution_result
    if exec_result and not exec_result.success:
        if not exec_result.install_success:
            gaps.append(Gap(description=f"Dependency install failed: {exec_result.install_stderr[-200:]}", severity=GapSeverity.BLOCKER, category="dependency"))
        elif "SyntaxError" in (exec_result.stderr or ""):
            gaps.append(Gap(description="Syntax error in code", severity=GapSeverity.BLOCKER, category="functionality"))
        elif "ModuleNotFoundError" in (exec_result.stderr or ""):
            gaps.append(Gap(description=f"Missing dependency: {exec_result.stderr.split('ModuleNotFoundError')[-1][:100]}", severity=GapSeverity.BLOCKER, category="dependency"))
        else:
            gaps.append(Gap(description=f"Execution failed (exit {exec_result.exit_code})", severity=GapSeverity.BLOCKER, category="functionality", suggested_fix=exec_result.error_summary[:200]))
...
def _build_report(gaps: list[Gap], confidence: float) -> GapReport:
    return GapReport(
        ...
        summary=f"Deterministic analysis: {len(gaps)} issue(s) found" if gaps else "No issues detected",
    )
