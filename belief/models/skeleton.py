...
class ModelFieldSpec(BaseModel):
    ...
    type_annotation: str = Field(
        description="Python type as string, e.g. 'Optional[str]', 'list[float]'"
    )
    default: Optional[str] = Field(
        default=None, description="Default value as string, or None if required"
    )
...
class ModelSpec(BaseModel):
    ...
    base_class: str = Field(
        default="BaseModel", description="Parent class (BaseModel, or a prior model in the chain)"
    )
...
class ABCDefinition(BaseModel):
    ...
    class_attributes: list[ModelFieldSpec] = Field(
        default_factory=list, description="Class-level attributes with type annotations"
    )
    docstring: Optional[str] = None
...
class ConfigSchema(BaseModel):
    ...
    env_prefix: Optional[str] = Field(
        default=None, description="Pydantic Settings env prefix, e.g. 'PIPELINE_'"
    )
    docstring: Optional[str] = None
...
class EndpointContract(BaseModel):
    ...
    request_model: Optional[str] = Field(
        default=None, description="Pydantic model name for request body"
    )
    response_model: Optional[str] = Field(
        default=None, description="Pydantic model name for response"
    )
    status_code: int = Field(default=200, description="Expected success status code")
    error_codes: list[int] = Field(
        default_factory=list, description="Expected error status codes, e.g. [404, 422]"
    )
...
class CLIContract(BaseModel):
    ...
    options: list[str | dict] = Field(
        default_factory=list, description="Optional flags, e.g. ['--verbose', '--output FILE']"
    )
    exit_codes: list[int | dict] = Field(
        default_factory=lambda: [0], description="Expected exit codes"
    )
...
class FileTreeEntry(BaseModel):
    ...
    estimated_lines: Optional[int] = Field(
        default=None, description="Rough estimate of implementation line count"
    )
...
class SkeletonArtifact(BaseModel):
    ...
    dependency_edges: list[DependencyEdge] = Field(
        default_factory=list, description="All file-to-file dependency edges"
    )

    # Typed interfaces (Pass 1 content)
    model_chains: list[ModelChain] = Field(
        default_factory=list, description="Progressive Pydantic model chains"
    )
    abc_definitions: list[ABCDefinition] = Field(
        default_factory=list, description="Abstract base classes"
    )
    protocol_definitions: list[ProtocolDefinition] = Field(
        default_factory=list, description="Protocol definitions for structural subtyping"
    )
    config_schemas: list[ConfigSchema] = Field(
        default_factory=list, description="Configuration/settings classes"
    )
    exception_specs: list[ExceptionSpec] = Field(
        default_factory=list, description="Custom exception hierarchy"
    )
    ...
    # Metadata
    entry_point: Optional[str] = Field(default=None, description="Main entry point file path")
